...
- 整理 GitHub Release / `RELEASE_NOTES.md` 时，必须严格以“上一个已发布版本”为对比基线
- 发版前必须先读取并对比“上一个已发布版本”的 GitHub Release / `RELEASE_NOTES.md` 文案，检查本次准备写入的条目是否已经在上一版对外发布过
- 如果某条内容在上一个版本说明里已经写过，除非这次是在上次已发布能力基础上的新增独立优化或新问题修复，否则不能再次写入本次发布说明
- 发布说明只能描述：
  - 本次版本相对上一个已发布版本，用户实际新增获得的功能
  - 本次版本相对上一个已发布版本，用户实际能感知到的修改、优化和问题修复
- 不要把“同一版本开发过程中的中间调整”写进发布说明
- 不要把“为了把本次新功能打磨到可发布而做的内部修正过程”拆成单独条目
- 如果某个点对用户升级到本版本后的感知没有独立意义，就不要写进发布说明
- 发布说明应优先回答“用户从上个版本升级到这个版本后，多了什么、变好了什么”

- 整理 CSDN 帖子时，必须严格以“上一次已发布帖子基线”为对比范围
- CSDN 帖子只能描述：
  - 相对上一篇帖子之后，新增了哪些功能
  - 相对上一篇帖子之后，优化了哪些已经发布过的能力
  - 相对上一篇帖子之后，修复了哪些用户可感知的问题
- 不要把同一轮开发中的内部调试、临时取舍、过程性改动写进帖子
- 帖子内容要按“相对上一篇帖子，项目又新增了什么、优化了什么”来组织
- CSDN 帖子正文必须始终带上项目地址：
  - `https://github.com/vhukze/DataDjinn`
...
- 用户说“发布 CSDN 帖子”时，默认只生成或更新 `docs/csdn_post.md`，并向用户说明正文内容和截图插入位置。
- 不得自行登录 CSDN、打开发布流程、填写文章、上传截图或提交发布；发布动作由用户本人手动完成。
- 除非用户明确要求代为执行具体外部发布操作并在操作前再次确认，否则不得通过浏览器或其他外部工具向 CSDN 发送内容。
- 生成文章时必须列出截图的绝对路径，并标明每张截图在正文中的插入位置。
...
## 18.1 测试范围硬规则
- 日常修改问题或新增功能时，只运行本次改动涉及模块的相关测试，必须包含对应的 bug 用例和直接相关的现有回归用例；不要求运行整套发版用例
- 正式发版前，才运行仓库中全部通用功能用例并保证通过
...
## 19. UTF-8 编辑硬规则
- 所有源码文件必须保持 UTF-8 编码
- 严禁使用可能破坏源码编码的编辑方式，尤其禁止用 PowerShell 文本重定向、`Out-File`、`Set-Content`、无编码保证的整文件覆盖、猜测编码后再转码这类方式直接修改源码
- 默认只允许使用 `apply_patch` 或明确保证 UTF-8 的安全编辑方式修改代码文件
- 如果发现文件已经出现乱码，禁止继续做编码猜测和反向转码试错，必须先停止高风险操作，再用最小改动方式恢复

## 20. 自动化测试执行规则
- 项目内编译命令属于正常开发和验证流程，无需单独向用户确认；按任务需要直接执行编译、类型检查和自动化测试。
- 以后解决任何 bug，都必须先用自动化测试覆盖并验证修复结果，确认问题被真实解决后，才能告诉用户“已解决”，禁止只凭猜测改代码
- 每次新增功能、修复问题或调整已有实现时，必须同步补充对应的自动化测试，至少覆盖本次改动涉及的用户可见行为和已知回归点
- 每次改动完成后，结束前必须运行“当前改动涉及模块”的相关测试并确认通过，不能只改代码不验证
- 前端改动默认至少执行：
  - `npm run typecheck:web`
  - 当前改动涉及模块的回归测试用例
- 如果改动涉及 Electron 主进程、预加载层、构建入口或 `src/main` 相关代码，还必须额外执行：
  - `npm run typecheck:node`
- 如果测试失败，必须先修复问题或同步更新测试，确认通过后才能结束本轮任务

## 21. 模块级测试补跑规则
- 以后修改任何问题后，除了补充本次对应的新增自动化测试，还必须把当前改动涉及模块中“与本次改动直接相关的现有自动化测试用例”一起跑一遍并通过
- 不要求每次都把整个仓库所有测试全跑一遍，但本次实际跑到的相关用例必须全部通过，不能带着失败结束
- 例如修改表格模块时，至少要把表格中与本次改动相关的搜索、选择、编辑、刷新、滚动等现有回归用例一起跑通，再算本轮改动完成

## 22. Bug 修复的测试验证规则
- 处理 bug 时，先判断此 bug 是否涉及以前修改过的功能或模块
- 如果已有对应的自动化测试用例，就用原有用例验证修复是否引入回归
- 如果此 bug 没有对应的自动化测试用例，则必须新增用例覆盖该 bug 的用户可见行为，并用新增用例验证修复
- 修复完成后，必须运行当前改动涉及模块的相关回归测试并通过，才能结束本轮 bug 修复任务
- 测试用例要区分为“改 bug 用例”和“通用功能用例”
- 发版前必须跑一遍所有“通用功能用例”，不要求把历史所有“改 bug 用例”全部跑一遍

## 23. AI 产品知识库同步规则
- AI 内置产品知识库位于 `backend/app/ai/product_knowledge.py`
- 后续新增、修改或移除任何用户可见的软件功能、设置项、快捷键、导入导出流程、驱动能力、界面入口或操作限制时，必须同步更新该知识库
- 同步更新时必须补充或调整自动化测试，至少验证对应内容会进入 AI 系统提示词，确保未选择数据库上下文时 AI 仍能正确回答产品用法问题
