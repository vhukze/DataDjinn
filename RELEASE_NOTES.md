...
- 修复应用长时间空闲后首次双击分组会卡顿约十秒且不展开的问题；现在首次操作即可完成展开或收起。
- 统一连接树展开状态的同步时机，避免首个节点操作读取旧状态；分组选择不再触发无关的 AI 上下文重算。
- 补充分组首次展开、分组层级、置顶和拖拽排序的回归覆盖。

**Full Changelog**: https://github.com/vhukze/DataDjinn/compare/v0.3.17...v0.3.18
