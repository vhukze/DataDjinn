...
- 安全与体验：Windows DPAPI 本机密码保护、浅色/深色主题、自定义标题栏、后台更新检测；安装版通过标准安装器自动更新并重新打开，更新入口不再启动命令行窗口。
...
| 本机 MCP 服务 | 通过 STDIO 向本机 Agent 暴露已授权的连接和数据库工具 | 未安装且关闭，可独立更新 |
...
关系型连接可在新建或编辑时开启 Git 版本管理：MySQL、ClickHouse 按数据库纳管，PostgreSQL、高斯、达梦和 Oracle 按模式纳管，SQLite 自动管理当前库。首次创建基线会把所选范围内的 DDL 和压缩数据作为一次提交上传；后续结构变更在后台创建库级快照，表预览保存数据则只压缩并提交变化表。可从连接旁 Git 图标或右键菜单查看提交概要和变更 SQL；表预览左侧 Git 图标可查看表级历史、逐行逐字段数据差异，并恢复历史结构或数据。后台任务入口会显示当前 Git 提交进度，支持停止任务。
...
基础安装包内置 Python 运行时和基础后端依赖；AI、MCP 和 JDBC 等可选功能在首次使用时按需下载安装。
