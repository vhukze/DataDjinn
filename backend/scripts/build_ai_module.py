...
BUILD_DIR = BACKEND_DIR / "build" / "ai-module"
MODULE_NAME = "datadjinn-ai"
MODULE_VERSION = os.environ.get("DATADJINN_AI_MODULE_VERSION", "1.0.0")
...
def main() -> None:
    if sys.platform != "win32":
        raise RuntimeError("AI 模块当前仅生成 Windows x64 包")
    ...
    run([
        ...
        "--add-data", f"{VENDOR_JPYPE_DIR}{PYINSTALLER_DATA_SEPARATOR}vendor/jpype15",
        "--collect-submodules", "jpype",
        "--hidden-import", "openai",
        "--hidden-import", "pymongo",
        "--hidden-import", "redis",
        "--hidden-import", "clickhouse_connect",
        "--collect-submodules", "clickhouse_connect",
    ...
    if not generated_root.exists():
        raise RuntimeError(f"AI 模块构建产物不存在：{generated_root}")
    generated_root.rename(MODULE_ROOT)
    executable = MODULE_ROOT / f"{MODULE_NAME}.exe"
    if not executable.exists():
        raise RuntimeError(f"AI 模块缺少启动程序：{executable}")
    ...
    (MODULE_ROOT / "module.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
