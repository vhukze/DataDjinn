...
class DatabaseAgentStreamingTests(unittest.TestCase):
    ...
    def test_system_prompt_includes_product_knowledge_for_global_help(self) -> None:
        ...
        self.assertIn("有主键或单列唯一键的表会保存行级标识", prompt)
        self.assertIn("Git 表数据版本管理", prompt)
        self.assertIn("基础版本未安装该扩展时只保留结构版本与同步能力", prompt)
        self.assertIn("安装后可选择表创建首个数据快照", prompt)
        self.assertIn("标准表预览保存修改会在后台自动生成新快照", prompt)
        self.assertIn("当前版本不提供表数据回退或大表外部存储", prompt)
        self.assertNotIn("受控回退", prompt)
        self.assertIn("达梦和 Oracle 需选择要纳管的模式", prompt)
        self.assertIn("未选中的范围不会生成结构快照", prompt)
        ...
        self.assertIn("首次打开连接以及通过界面创建、删除或修改库表结构时", prompt)
