...
type ResourceTreePanelProps = {
  ...
  updateDragOverFolderTarget: (
    value: { folderId: string; zone: 'before' | 'after' } | undefined
  ) => void
  ...
  setTreeContextMenu: React.Dispatch<
    React.SetStateAction<{ x: number; y: number; node: DatabaseTreeNode } | null>
  >
  ...
  openConnectionById: (
    connectionId: string
  ) => Promise<{ is_open?: boolean; database_type?: string } | undefined>
...
const ResourceTreePanel = memo(
  function ResourceTreePanel({
    ...
    const treeContextMenuItems = treeContextMenu
      ? getTreeContextMenuItems(treeContextMenu.node)
      : []
    ...
    useLayoutEffect(() => {
      ...
      const updatePosition = (): void => {
        ...
        const nextPosition = clampTreeContextMenuPosition(
          treeContextMenu.x,
          treeContextMenu.y,
          width,
          height
          ...
          >
            {!connectionsInitialized || !backendReady ? (
              <div className="resource-tree-loading-state">
                <div className="resource-tree-loading-spinner">
              ...
              <Alert
                message="暂无数据库连接或分组"
                description="先创建一个连接，或者新建分组开始整理。"
                type="info"
                showIcon
              />
            ) : (
              <Tree
                ...
                onDragOver={({ event }) => {
                  if (draggingConnectionIdsRef.current.length === 0) {
                    const target = event.target as HTMLElement | null
                    const draggedConnectionElement =
                ...
                switcherIcon={(nodeProps) => {
                  ...
                  return (
                    <span className="tree-switcher-glyph" aria-hidden="true">
                      <span
                        className={`tree-switcher-orb${nodeProps.expanded ? ' is-expanded' : ''}`}
                      >
                ...
                onRightClick={({ node, event }) => {
                  ...
                  if (
                    ...
                    treeNode.connectionId &&
                    !selectedConnectionIds.includes(treeNode.connectionId)
                  ) {
                ...
                onDoubleClick={(event, node) => {
                  ...
                  if (
                    treeNode.kind === 'database' &&
                    treeNode.connectionId &&
                    treeNode.databaseName &&
                    getConnection(treeNode.connectionId)?.database_type === 'redis'
                  ) {
                  ...
                  if (
                    (treeNode.kind === 'table' || treeNode.kind === 'db-object') &&
                    treeNode.connectionId &&
                    treeNode.tableName &&
                    (treeNode.objectType === 'table' || treeNode.objectType === 'view')
                  ) {
                    activateAIContextFromNode(treeNode)
                    const connection = getConnection(treeNode.connectionId)
                    if (connection?.database_type === 'redis') {
                      void openRedisDatabaseBrowser(
                        treeNode.connectionId,
                        treeNode.databaseName ?? getDefaultDatabaseName(connection) ?? 'db0'
                      )
                    } else {
