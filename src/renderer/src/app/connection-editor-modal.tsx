...
export const ConnectionEditorModal = memo(function ConnectionEditorModal({
  ...
  return (
    <Modal
      ...
      {...FAST_MODAL_PROPS}
    >
      <Form form={form} layout="vertical" className={`connection-editor-form${splitLayout ? ' connection-editor-form-split' : ''}`}>
          <Form.Item name="database_type" style={{ display: 'none' }}><Input /></Form.Item>
          <div className={`connection-editor-layout${splitLayout ? '' : ' connection-editor-layout-single'}`}>
              ...
              <div className="connection-editor-side">
                <div className="connection-editor-side-card">
                  <div className="connection-editor-section-head">
                    <Typography.Title level={5} className="connection-editor-section-title">SSH 隧道</Typography.Title>
                  ...
                  <Form.Item name="ssh_enabled" label="启用 SSH 隧道" valuePropName="checked">
                    <Switch checkedChildren="已启用" unCheckedChildren="已关闭" onChange={handleSshEnabledChange} />
                  </Form.Item>
                  <div className={`connection-editor-ssh-fields${sshEnabled ? '' : ' is-hidden'}`}>
                    <div className="connection-editor-inline-fields">
                      <Form.Item name="ssh_host" label="SSH 主机" rules={[{ required: sshEnabled, message: '请输入 SSH 主机' }]}>
                        <Input />
                      </Form.Item>
                      <Form.Item name="ssh_port" label="SSH 端口" initialValue={22} rules={[{ required: sshEnabled, message: '请输入 SSH 端口' }]}>
                        <InputNumber min={1} max={65535} className="full-width" placeholder="22" />
                      </Form.Item>
                    </div>
                    <Form.Item name="ssh_username" label="SSH 用户名" rules={[{ required: sshEnabled, message: '请输入 SSH 用户名' }]}>
                      <Input placeholder="请输入 SSH 用户名" />
                    </Form.Item>
                    <Form.Item name="ssh_auth_type" label="认证方式" initialValue="password" rules={[{ required: sshEnabled, message: '请选择 SSH 认证方式' }]}>
                      <Select
                        options={[
                    ...
                    <div className={sshAuthType === 'private_key' ? '' : 'connection-editor-ssh-auth-hidden'}>
                      <Form.Item name="ssh_private_key_path" label="私钥路径" rules={[{ required: sshEnabled && sshAuthType === 'private_key', message: '请输入私钥路径' }]}>
                    ...
                    <div className={sshAuthType === 'private_key' ? 'connection-editor-ssh-auth-hidden' : ''}>
                      <Form.Item name="ssh_password" label="SSH 密码" rules={[{ required: sshEnabled && sshAuthType === 'password', message: '请输入 SSH 登录密码' }]}>
                        <Input.Password placeholder="请输入 SSH 登录密码" />
                      </Form.Item>
                    </div>
                    <div className="connection-editor-side-actions">
                      <Button className="connection-editor-ssh-test-btn" loading={testingSshConnection} onClick={onTestSshConnection}>
                        测试 SSH
                      </Button>
                    </div>
                  </div>
